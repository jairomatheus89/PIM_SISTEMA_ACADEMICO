...
class AlunoScreen(Frame):
    def __init__(self, master, voltar_callback):
        ...
        # Botões
        self.btn_atividades = Button(self.menu_frame, text="Atividades", bg="#D4A017", width=15, font=("Arial", 12, "bold"), command=lambda: self.mostrar_frame("listar_atividades"))
        self.btn_atividades.grid(row=0, column=0, padx=5)

        self.btn_notas = Button(self.menu_frame, text="Notas", bg="#D4A017", width=15, font=("Arial", 12, "bold"), command=lambda: self.mostrar_frame("listar_notas"))
        self.btn_notas.grid(row=0, column=1, padx=5)

        self.btn_voltar = Button(self.menu_frame, text="Voltar", bg="#D4A017", width=15, font=("Arial", 12, "bold"), command=self.voltar_elimpar)
...
# Treeview style
    ...
    def listar_atividades(self):
        ...
        Label(frame_materia, text="Selecione a Matéria:", font=("Calibri", 12, "bold"),
        ...
        # Frame para Treeview
        self.tree_frame = Frame(frame_atividades, bg="#0D1B2A")
    ...
    def atualizar_tabela(self):
        ...
        for i, atividade in enumerate(self.atividades_completas):
            ...
            self.tree.tag_configure(f"linha_{i}", foreground="white")
            self.tree.item(item, tags=(f"linha_{i}", f"status_{i}"))
            self.tree.tag_configure(f"status_{i}")
    ...
    def listar_notas(self):
        ...
        Label(frame_notas, text="Selecione a Matéria:", font=("Calibri", 12, "bold"), bg="#0D1B2A", fg="#F0F0F0").pack(pady=5)
        ...
        self.tree_frame_notas = Frame(frame_notas, bg="#0D1B2A", width=self.master.winfo_width(), height=60)  # altura aproximada para 2 linhas
    ...
    def carregar_notas(self):
        ...
        largura_total = self.tree_frame_notas.winfo_width() or 600  # largura total disponível
        largura_coluna = int(largura_total / len(colunas))          # divide igualmente entre colunas

        for col in colunas:
            self.tree_notas.column(col, anchor=CENTER, width=largura_coluna, stretch=True)
